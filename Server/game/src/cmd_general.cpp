...
ACMD(do_user_horse_ride)
{
	...
	if (ch->IsHorseRiding() == false)
	{
		...
		if (ch->GetMountVnum())
		{
			ch->ChatPacketTrans(CHAT_TYPE_INFO, "ÀÌ¹Ì Å»°ÍÀ» ÀÌ¿ëÁßÀÔ´Ï´Ù.");
			return;
		}

		if (ch->GetHorse() == NULL)
		{
			ch->ChatPacketTrans(CHAT_TYPE_INFO, "¸»À» ¸ÕÀú ¼ÒÈ¯ÇØÁÖ¼¼¿ä.");
			return;
		}
...
ACMD(do_user_horse_back)
{
	if (ch->GetHorse() != NULL)
	{
		...
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "¸»À» µ¹·Áº¸³Â½À´Ï´Ù.");
	}
	else if (ch->IsHorseRiding() == true)
	{
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "¸»¿¡¼­ ¸ÕÀú ³»·Á¾ß ÇÕ´Ï´Ù.");
	}
	else
	{
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "¸»À» ¸ÕÀú ¼ÒÈ¯ÇØÁÖ¼¼¿ä.");
...
ACMD(do_user_horse_feed)
{
	...
	if (ch->GetHorse() == NULL)
	{
		if (ch->IsHorseRiding() == false)
			...
			ch->ChatPacketTrans(CHAT_TYPE_INFO, "¸»À» Åº »óÅÂ¿¡¼­´Â ¸ÔÀÌ¸¦ ÁÙ ¼ö ¾ø½À´Ï´Ù.");
		return;
	}
	...
	if (ch->CountSpecifyItem(dwFood) > 0)
	{
		...
		ch->FeedHorse();
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "¸»¿¡°Ô %s%s ÁÖ¾ú½À´Ï´Ù.",
				ITEM_MANAGER::instance().GetTable(dwFood)->szLocaleName,
				"");
	}
	else
	{
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "%s ¾ÆÀÌÅÛÀÌ ÇÊ¿äÇÕ´Ï´Ù", ITEM_MANAGER::instance().GetTable(dwFood)->szLocaleName);
	}
}
...
EVENTFUNC(timed_event)
{
	...
	{
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "%dÃÊ ³²¾Ò½À´Ï´Ù.", info->left_second);
		--info->left_second;
	}
...
ACMD(do_cmd)
{
	/* RECALL_DELAY
	   ...
	   {
	   ch->ChatPacketTrans(CHAT_TYPE_INFO, "Ãë¼Ò µÇ¾ú½À´Ï´Ù.");
	...
	{
		case SCMD_LOGOUT:
			ch->ChatPacketTrans(CHAT_TYPE_INFO, "·Î±×ÀÎ È­¸éÀ¸·Î µ¹¾Æ °©´Ï´Ù. Àá½Ã¸¸ ±â´Ù¸®¼¼¿ä.");
			break;

		case SCMD_QUIT:
			ch->ChatPacketTrans(CHAT_TYPE_INFO, "°ÔÀÓÀ» Á¾·á ÇÕ´Ï´Ù. Àá½Ã¸¸ ±â´Ù¸®¼¼¿ä.");
			break;

		case SCMD_PHASE_SELECT:
			ch->ChatPacketTrans(CHAT_TYPE_INFO, "Ä³¸¯ÅÍ¸¦ ÀüÈ¯ ÇÕ´Ï´Ù. Àá½Ã¸¸ ±â´Ù¸®¼¼¿ä.");
			break;
	}
...
ACMD(do_mount)
{
	...
	if (distance > 600.0f)
	{
	ch->ChatPacketTrans(CHAT_TYPE_INFO, "Á» ´õ °¡±îÀÌ °¡¼­ ³»¸®¼¼¿ä.");
	return;
	}
	...
	{
	ch->ChatPacketTrans(CHAT_TYPE_INFO, "°Å±â¿¡´Â Å» ¼ö ¾ø¾î¿ä.");
	return;
	}
	...
	if (distance > 600.0f)
	{
	ch->ChatPacketTrans(CHAT_TYPE_INFO, "Á» ´õ °¡±îÀÌ °¡¼­ Å¸¼¼¿ä.");
	return;
	}
	...
	if (subcmd != SCMD_RESTART_TOWN && (!ch->GetWarMap() || ch->GetWarMap()->GetType() == GUILD_WAR_TYPE_FLAG))
	{
		if (!test_server)
		{
			if (ch->IsHack())
			{
				...
				{
					ch->ChatPacketTrans(CHAT_TYPE_INFO, "¾ÆÁ÷ Àç½ÃÀÛ ÇÒ ¼ö ¾ø½À´Ï´Ù. (%dÃÊ ³²À½)", iTimeToDead - (180 - g_nPortalLimitTime));
					return;
				}
			}
#define eFRS_HERESEC	170
			...
			{
				ch->ChatPacketTrans(CHAT_TYPE_INFO, "¾ÆÁ÷ Àç½ÃÀÛ ÇÒ ¼ö ¾ø½À´Ï´Ù. (%dÃÊ ³²À½)", iTimeToDead - eFRS_HERESEC);
				return;
			}
		}
	}
...
#define eFRS_TOWNSEC	173
		...
		{
			ch->ChatPacketTrans(CHAT_TYPE_INFO, "¾ÆÁ÷ ¸¶À»¿¡¼­ Àç½ÃÀÛ ÇÒ ¼ö ¾ø½À´Ï´Ù. (%d ÃÊ ³²À½)", iTimeToDead - eFRS_TOWNSEC);
			return;
		}
	}
	//END_PREVENT_HACK
	...
	if (1 == quest::CQuestManager::instance().GetEventFlag("threeway_war"))
	{
		if (subcmd == SCMD_RESTART_TOWN || subcmd == SCMD_RESTART_HERE)
		{
			...
			if (true == CThreeWayWar::instance().IsSungZiMapIndex(ch->GetMapIndex()))
			{
				...
				{
					ch->ChatPacketTrans(CHAT_TYPE_INFO, "¼ºÁö¿¡¼­ ºÎÈ° ±âÈ¸¸¦ ¸ðµÎ ÀÒ¾ú½À´Ï´Ù! ¸¶À»·Î ÀÌµ¿ÇÕ´Ï´Ù!");
					ch->WarpSet(EMPIRE_START_X(ch->GetEmpire()), EMPIRE_START_Y(ch->GetEmpire()));
				}
				else
				{
...
ACMD(do_stat_minus)
{
	...
	if (ch->IsPolymorphed())
	{
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "µÐ°© Áß¿¡´Â ´É·ÂÀ» ¿Ã¸± ¼ö ¾ø½À´Ï´Ù.");
		return;
	}
...
ACMD(do_pvp)
{
	...
	{
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "´ë·ÃÀå¿¡¼­ »ç¿ëÇÏ½Ç ¼ö ¾ø½À´Ï´Ù.");
		return;
	}
	...
	{
		pkVictim->ChatPacketTrans(CHAT_TYPE_INFO, "»ó´ë¹æÀÌ ´ë·ÃÁßÀÔ´Ï´Ù.");
		return;
	}
...
ACMD(do_guildskillup)
{
	...
	if (!ch->GetGuild())
	{
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "<±æµå> ±æµå¿¡ ¼ÓÇØÀÖÁö ¾Ê½À´Ï´Ù.");
		return;
	}
	...
	{
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "<±æµå> ±æµå ½ºÅ³ ·¹º§À» º¯°æÇÒ ±ÇÇÑÀÌ ¾ø½À´Ï´Ù.");
	}
}
...
ACMD(do_safebox_change_password)
{
	...
	{
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "<Ã¢°í> Àß¸øµÈ ¾ÏÈ£¸¦ ÀÔ·ÂÇÏ¼Ì½À´Ï´Ù.");
		return;
	}
...
ACMD(do_mall_password)
{
	...
	if (ch->GetMall())
	{
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "<Ã¢°í> Ã¢°í°¡ ÀÌ¹Ì ¿­·ÁÀÖ½À´Ï´Ù.");
		return;
	}

	if (iPulse - ch->GetMallLoadTime() < passes_per_sec * 10) // 10ÃÊ¿¡ ÇÑ¹ø¸¸ ¿äÃ» °¡´É
	{
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "<Ã¢°í> Ã¢°í¸¦ ´ÝÀºÁö 10ÃÊ ¾È¿¡´Â ¿­ ¼ö ¾ø½À´Ï´Ù.");
		return;
	}
...
ACMD(do_ungroup)
{
	...
	{
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "<ÆÄÆ¼> ¼­¹ö ¹®Á¦·Î ÆÄÆ¼ °ü·Ã Ã³¸®¸¦ ÇÒ ¼ö ¾ø½À´Ï´Ù.");
		return;
	}

	if (ch->GetDungeon())
	{
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "<ÆÄÆ¼> ´øÀü ¾È¿¡¼­´Â ÆÄÆ¼¿¡¼­ ³ª°¥ ¼ö ¾ø½À´Ï´Ù.");
		return;
	}
	...
	{
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "<ÆÄÆ¼> ÆÄÆ¼¿¡¼­ ³ª°¡¼Ì½À´Ï´Ù.");
...
ACMD(do_war)
{
	...
	if (g->UnderAnyWar())
	{
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "<±æµå> ÀÌ¹Ì ´Ù¸¥ ÀüÀï¿¡ ÂüÀü Áß ÀÔ´Ï´Ù.");
		return;
	}
	...
	{
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "<±æµå> ±æµåÀü¿¡ ´ëÇÑ ±ÇÇÑÀÌ ¾ø½À´Ï´Ù.");
		return;
	}
	...
	if (!opp_g)
	{
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "<±æµå> ±×·± ±æµå°¡ ¾ø½À´Ï´Ù.");
		return;
	}
	...
	switch (g->GetGuildWarState(opp_g->GetID()))
	{
		case GUILD_WAR_NONE:
			{
				...
				{
					ch->ChatPacketTrans(CHAT_TYPE_INFO, "<±æµå> »ó´ë¹æ ±æµå°¡ ÀÌ¹Ì ÀüÀï Áß ÀÔ´Ï´Ù.");
					return;
				}
				...
				{
					ch->ChatPacketTrans(CHAT_TYPE_INFO, "<±æµå> Àüºñ°¡ ºÎÁ·ÇÏ¿© ±æµåÀüÀ» ÇÒ ¼ö ¾ø½À´Ï´Ù.");
					return;
				}

				if (opp_g->GetGuildMoney() < iWarPrice)
				{
					ch->ChatPacketTrans(CHAT_TYPE_INFO, "<±æµå> »ó´ë¹æ ±æµåÀÇ Àüºñ°¡ ºÎÁ·ÇÏ¿© ±æµåÀüÀ» ÇÒ ¼ö ¾ø½À´Ï´Ù.");
					return;
				}
			}
			break;
		...
		case GUILD_WAR_SEND_DECLARE:
			{
				ch->ChatPacketTrans(CHAT_TYPE_INFO, "ÀÌ¹Ì ¼±ÀüÆ÷°í ÁßÀÎ ±æµåÀÔ´Ï´Ù.");
				return;
			}
			break;
		...
		case GUILD_WAR_RESERVE:
			{
				ch->ChatPacketTrans(CHAT_TYPE_INFO, "<±æµå> ÀÌ¹Ì ÀüÀïÀÌ ¿¹¾àµÈ ±æµå ÀÔ´Ï´Ù.");
				return;
			}
			break;
		...
		default:
			ch->ChatPacketTrans(CHAT_TYPE_INFO, "<±æµå> ÀÌ¹Ì ÀüÀï ÁßÀÎ ±æµåÀÔ´Ï´Ù.");
			g->RequestRefuseWar(opp_g->GetID());
			return;
	}
	...
	if (!g->CanStartWar(type))
	{
		...
		{
			ch->ChatPacketTrans(CHAT_TYPE_INFO, "<±æµå> ·¹´õ Á¡¼ö°¡ ¸ðÀÚ¶ó¼­ ±æµåÀüÀ» ÇÒ ¼ö ¾ø½À´Ï´Ù.");
			sys_log(0, "GuildWar.StartError.NEED_LADDER_POINT");
		}
		else if (g->GetMemberCount() < GUILD_WAR_MIN_MEMBER_COUNT)
		{
			ch->ChatPacketTrans(CHAT_TYPE_INFO, "<±æµå> ±æµåÀüÀ» ÇÏ±â À§ÇØ¼± ÃÖ¼ÒÇÑ %d¸íÀÌ ÀÖ¾î¾ß ÇÕ´Ï´Ù.", GUILD_WAR_MIN_MEMBER_COUNT);
	...
	if (!opp_g->CanStartWar(GUILD_WAR_TYPE_FIELD))
	{
		if (opp_g->GetLadderPoint() == 0)
			ch->ChatPacketTrans(CHAT_TYPE_INFO, "<±æµå> »ó´ë¹æ ±æµåÀÇ ·¹´õ Á¡¼ö°¡ ¸ðÀÚ¶ó¼­ ±æµåÀüÀ» ÇÒ ¼ö ¾ø½À´Ï´Ù.");
		else if (opp_g->GetMemberCount() < GUILD_WAR_MIN_MEMBER_COUNT)
			ch->ChatPacketTrans(CHAT_TYPE_INFO, "<±æµå> »ó´ë¹æ ±æµåÀÇ ±æµå¿ø ¼ö°¡ ºÎÁ·ÇÏ¿© ±æµåÀüÀ» ÇÒ ¼ö ¾ø½À´Ï´Ù.");
		return;
	}
	...
	do
	{
		...
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "<±æµå> »ó´ë¹æ ±æµåÀÇ ±æµåÀåÀÌ Á¢¼ÓÁßÀÌ ¾Æ´Õ´Ï´Ù.");
		g->RequestRefuseWar(opp_g->GetID());
		return;
...
ACMD(do_messenger_auth)
{
	...
	if (bIsAdded && bIsDenied)
	{
		...
		if (tch)
			tch->ChatPacketTrans(CHAT_TYPE_INFO, "%s ´ÔÀ¸·Î ºÎÅÍ Ä£±¸ µî·ÏÀ» °ÅºÎ ´çÇß½À´Ï´Ù.", ch->GetName());
	}
...
ACMD(do_view_equip)
{
	...
	if (*arg1)
	{
		...
		/*
		   ...
		   if (ch->GetSP() < iSPCost)
		   {
		   ch->ChatPacketTrans(CHAT_TYPE_INFO, "Á¤½Å·ÂÀÌ ºÎÁ·ÇÏ¿© ´Ù¸¥ »ç¶÷ÀÇ Àåºñ¸¦ º¼ ¼ö ¾ø½À´Ï´Ù.");
		   return;
		   }
...
ACMD(do_party_request)
{
	...
	if (ch->GetParty())
	{
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "ÀÌ¹Ì ÆÄÆ¼¿¡ ¼ÓÇØ ÀÖÀ¸¹Ç·Î °¡ÀÔ½ÅÃ»À» ÇÒ ¼ö ¾ø½À´Ï´Ù.");
		return;
	}
...
ACMD(do_monarch_warpto)
{
	...
	{
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "±ºÁÖ¸¸ÀÌ »ç¿ë °¡´ÉÇÑ ±â´ÉÀÔ´Ï´Ù");
		return;
	}
	...
	{
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "%d ÃÊ°£ ÄðÅ¸ÀÓÀÌ Àû¿ëÁßÀÔ´Ï´Ù.", ch->GetMCLTime(CHARACTER::MI_WARP));
		return;
	}
	...
	if (!CMonarch::instance().IsMoneyOk(WarpPrice, ch->GetEmpire()))
	{
		...
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "±¹°í¿¡ µ·ÀÌ ºÎÁ·ÇÕ´Ï´Ù. ÇöÀç : %u ÇÊ¿ä±Ý¾× : %u", NationMoney, WarpPrice);
		return;
	}
	...
	if (!*arg1)
	{
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "»ç¿ë¹ý: warpto <character name>");
		return;
	}
	...
	if (!tch)
	{
		...
		if (pkCCI)
		{
			...
			{
				ch->ChatPacketTrans(CHAT_TYPE_INFO, "Å¸Á¦±¹ À¯Àú¿¡°Ô´Â ÀÌµ¿ÇÒ¼ö ¾ø½À´Ï´Ù");
				return;
			}

			if (pkCCI->bChannel != g_bChannel)
			{
				ch->ChatPacketTrans(CHAT_TYPE_INFO, "ÇØ´ç À¯Àú´Â %d Ã¤³Î¿¡ ÀÖ½À´Ï´Ù. (ÇöÀç Ã¤³Î %d)", pkCCI->bChannel, g_bChannel);
				return;
			}
			...
			else
			{
				...
				ch->ChatPacketTrans(CHAT_TYPE_INFO, "%s ¿¡°Ô·Î ÀÌµ¿ÇÕ´Ï´Ù", arg1);
				ch->WarpSet(pos.x, pos.y);
...
ACMD(do_monarch_transfer)
{
	...
	if (!*arg1)
	{
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "»ç¿ë¹ý: transfer <name>");
		return;
	}
	...
	{
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "%d ÃÊ°£ ÄðÅ¸ÀÓÀÌ Àû¿ëÁßÀÔ´Ï´Ù.", ch->GetMCLTime(CHARACTER::MI_TRANSFER));
		return;
	}
	...
	if (!tch)
	{
		...
		if (pkCCI)
		{
			if (pkCCI->bEmpire != ch->GetEmpire())
			{
				ch->ChatPacketTrans(CHAT_TYPE_INFO, "´Ù¸¥ Á¦±¹ À¯Àú´Â ¼ÒÈ¯ÇÒ ¼ö ¾ø½À´Ï´Ù.");
				return;
			}
			if (pkCCI->bChannel != g_bChannel)
			{
				ch->ChatPacketTrans(CHAT_TYPE_INFO, "%s ´ÔÀº %d Ã¤³Î¿¡ Á¢¼Ó Áß ÀÔ´Ï´Ù. (ÇöÀç Ã¤³Î: %d)", arg1, pkCCI->bChannel, g_bChannel);
				return;
			}
			if (!IsMonarchWarpZone(pkCCI->lMapIndex))
			{
			...
			ch->ChatPacketTrans(CHAT_TYPE_INFO, "%s ´ÔÀ» ¼ÒÈ¯ÇÏ¿´½À´Ï´Ù.", arg1);
		...
		{
			ch->ChatPacketTrans(CHAT_TYPE_INFO, "ÀÔ·ÂÇÏ½Å ÀÌ¸§À» °¡Áø »ç¿ëÀÚ°¡ ¾ø½À´Ï´Ù.");
		}
	...
	if (ch == tch)
	{
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "ÀÚ½ÅÀ» ¼ÒÈ¯ÇÒ ¼ö ¾ø½À´Ï´Ù.");
		return;
	}
...
ACMD(do_monarch_info)
{
	if (CMonarch::instance().IsMonarch(ch->GetPlayerID(), ch->GetEmpire()))
	{
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "³ªÀÇ ±ºÁÖ Á¤º¸");
		TMonarchInfo * p = CMonarch::instance().GetMonarch();
		for (int n = 1; n < 4; ++n)
		{
			if (n == ch->GetEmpire())
				ch->ChatPacketTrans(CHAT_TYPE_INFO, "[%s±ºÁÖ] : %s  º¸À¯±Ý¾× %lld ", EMPIRE_NAME(n), p->name[n], p->money[n]);
			else
				ch->ChatPacketTrans(CHAT_TYPE_INFO, "[%s±ºÁÖ] : %s  ", EMPIRE_NAME(n), p->name[n]);

		}
	}
	else
	{
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "±ºÁÖ Á¤º¸");
		TMonarchInfo * p = CMonarch::instance().GetMonarch();
		for (int n = 1; n < 4; ++n)
		{
			ch->ChatPacketTrans(CHAT_TYPE_INFO, "[%s±ºÁÖ] : %s  ", EMPIRE_NAME(n), p->name[n]);
...
ACMD(do_monarch_tax)
{
	...
	if (!ch->IsMonarch())
	{
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "±ºÁÖ¸¸ÀÌ »ç¿ëÇÒ¼ö ÀÖ´Â ±â´ÉÀÔ´Ï´Ù");
		return;
	}
	...
	if (tax < 1 || tax > 50)
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "1-50 »çÀÌÀÇ ¼öÄ¡¸¦ ¼±ÅÃÇØÁÖ¼¼¿ä");

	quest::CQuestManager::instance().SetEventFlag("trade_tax", tax);

	// ±ºÁÖ¿¡°Ô ¸Þ¼¼Áö ÇÏ³ª
	ch->ChatPacketTrans(CHAT_TYPE_INFO, "¼¼±ÝÀÌ %d %·Î ¼³Á¤µÇ¾ú½À´Ï´Ù");
...
ACMD(do_monarch_mob)
{
	...
	{
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "ÀÚ±¹ ¿µÅä¿¡¼­¸¸ »ç¿ëÇÒ ¼ö ÀÖ´Â ±â´ÉÀÔ´Ï´Ù");
		return;
	}
#endif
	...
	{
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "%d ÃÊ°£ ÄðÅ¸ÀÓÀÌ Àû¿ëÁßÀÔ´Ï´Ù.", ch->GetMCLTime(CHARACTER::MI_SUMMON));
		return;
	}
	...
	{
		int NationMoney = CMonarch::instance().GetMoney(ch->GetEmpire());
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "±¹°í¿¡ µ·ÀÌ ºÎÁ·ÇÕ´Ï´Ù. ÇöÀç : %u ÇÊ¿ä±Ý¾× : %u", NationMoney, SummonPrice);
		return;
	}
	...
	{
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "¼ÒÈ¯ÇÒ¼ö ¾ø´Â ¸ó½ºÅÍ ÀÔ´Ï´Ù. ¼ÒÈ¯°¡´ÉÇÑ ¸ó½ºÅÍ´Â È¨ÆäÀÌÁö¸¦ ÂüÁ¶ÇÏ¼¼¿ä");
		return;
	}
...
ACMD(do_dice)
{
	...
	if (ch->GetParty())
		...
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "´ç½ÅÀÌ ÁÖ»çÀ§¸¦ ±¼·Á %d°¡ ³ª¿Ô½À´Ï´Ù. (%d-%d)", n, start, end);
#endif
}
