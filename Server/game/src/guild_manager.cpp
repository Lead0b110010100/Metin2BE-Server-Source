...
DWORD CGuildManager::CreateGuild(TGuildCreateParameter& gcp)
{
	...
	{
		gcp.master->ChatPacketTrans(CHAT_TYPE_INFO, "<길드> 길드 이름이 적합하지 않습니다.");
		return 0;
	}
	...
	if (pmsg->Get()->uiNumRows > 0)
	{
		...
		if (!(row[0] && row[0][0] == '0'))
		{
			gcp.master->ChatPacketTrans(CHAT_TYPE_INFO, "<길드> 이미 같은 이름의 길드가 있습니다.");
			return 0;
		}
	}
	else
	{
		gcp.master->ChatPacketTrans(CHAT_TYPE_INFO, "<길드> 길드를 생성할 수 없습니다.");
		return 0;
	}
...
void CGuildManager::RefuseWar(DWORD guild_id1, DWORD guild_id2)
{
	...
	if (g1 && g2)
	{
		if (g2->GetMasterCharacter())
			g2->GetMasterCharacter()->ChatPacketTrans(CHAT_TYPE_INFO, "<길드> %s 길드가 길드전을 거부하였습니다.", g1->GetName());
	}
...
void CGuildManager::CancelWar(DWORD guild_id1, DWORD guild_id2)
{
	...
	if (g1)
	{
		...
		if (master1)
			master1->ChatPacketTrans(CHAT_TYPE_INFO, "<길드> 길드전이 취소 되었습니다.");
	}
	...
	if (g2)
	{
		...
		if (master2)
			master2->ChatPacketTrans(CHAT_TYPE_INFO, "<길드> 길드전이 취소 되었습니다.");
	}
