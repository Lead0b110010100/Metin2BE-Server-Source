...
bool CHARACTER::DoRefine(LPITEM item, bool bMoneyOnly)
{
	...
	if (RefineChance > 0)
	{
		...
		{
			ChatPacketTrans(CHAT_TYPE_INFO, "무료 개량 기회는 20 이하의 무기만 가능합니다");
			return false;
		}
	...
	if (result_vnum == 0)
	{
		ChatPacketTrans(CHAT_TYPE_INFO, "더 이상 개량할 수 없습니다.");
		return false;
	}
	...
	if (!pProto)
	{
		...
		ChatPacketTrans(CHAT_TYPE_INFO, "이 아이템은 개량할 수 없습니다.");
		return false;
	}
	...
	if (GetGold() < cost)
	{
		ChatPacketTrans(CHAT_TYPE_INFO, "개량을 하기 위한 돈이 부족합니다.");
		return false;
	}
	...
	if (!bMoneyOnly && !RefineChance)
	{
		for (int i = 0; i < prt->material_count; ++i)
		{
			if (CountSpecifyItem(prt->materials[i].vnum) < prt->materials[i].count)
			{
				...
				ChatPacketTrans(CHAT_TYPE_INFO, "개량을 하기 위한 재료가 부족합니다.");
				return false;
			}
		}
...
bool CHARACTER::DoRefineWithScroll(LPITEM item)
{
	...
	if (pkItemScroll->GetValue(0) == MUSIN_SCROLL)
	{
		...
		{
			ChatPacketTrans(CHAT_TYPE_INFO, "이 개량서로 더 이상 개량할 수 없습니다.");
			return false;
		}
	}
	// END_OF_MUSIC_SCROLL
	...
	else if (pkItemScroll->GetValue(0) == MEMO_SCROLL)
	{
		...
		{
			ChatPacketTrans(CHAT_TYPE_INFO, "이 개량서로 개량할 수 없습니다.");
			return false;
		}
	}
	else if (pkItemScroll->GetValue(0) == BDRAGON_SCROLL)
	{
		...
		{
			ChatPacketTrans(CHAT_TYPE_INFO, "이 아이템으로 개량할 수 없습니다.");
			return false;
		}
	}
...
bool CHARACTER::RefineInformation(BYTE bCell, BYTE bType, int iAdditionalCell)
{
	...
	{
		ChatPacketTrans(CHAT_TYPE_INFO, "사귀 타워 완료 보상은 한번까지 사용가능합니다.");
		return false;
	}
	// END_OF_REFINE_COST
	...
	if (item->GetType() == ITEM_USE && item->GetSubType() == USE_TUNING)
	{
		if (bType == 0)
		{
			ChatPacketTrans(CHAT_TYPE_INFO, "이 아이템은 이 방식으로는 개량할 수 없습니다.");
			return false;
		}
		else
		{
			...
			{
				ChatPacketTrans(CHAT_TYPE_INFO, "같은 개량서를 합칠 수는 없습니다.");
				ChatPacketTrans(CHAT_TYPE_INFO, "축복의 서와 현철을 합칠 수 있습니다.");
				return false;
			}
		}
	}
...
bool CHARACTER::RefineItem(LPITEM pkItem, LPITEM pkTarget)
{
	...
	else if (pkItem->GetSubType() == USE_DETACHMENT && IS_SET(pkTarget->GetFlag(), ITEM_FLAG_REFINEABLE))
	{
		...
		{
			ChatPacketTrans(CHAT_TYPE_INFO, "빼낼 수 있는 메틴석이 없습니다.");
			return false;
		}
	}
...
bool CHARACTER::GiveRecallItem(LPITEM item)
{
	...
	{
		ChatPacketTrans(CHAT_TYPE_INFO, "기억해 둘 수 없는 위치 입니다.");
		return false;
	}
	...
	{
		ChatPacketTrans(CHAT_TYPE_INFO, "소지품에 빈 공간이 없습니다.");
		return false;
	}
...
void CHARACTER::ProcessRecallItem(LPITEM item)
{
	...
	switch (idx)
	{
		...
		case 304:
			if( GetLevel() < 90 )
			{
				ChatPacketTrans(CHAT_TYPE_INFO, "아이템의 레벨 제한보다 레벨이 낮습니다.");
				return;
			}
			else
				break;
	...
	if (iEmpireByMapIndex && GetEmpire() != iEmpireByMapIndex)
	{
		ChatPacketTrans(CHAT_TYPE_INFO, "기억된 위치가 타제국에 속해 있어서 귀환할 수 없습니다.");
		item->SetSocket(0, 0);
		item->SetSocket(1, 0);
	}
	else
...
int CalculateConsume(LPCHARACTER ch)
{
	...
	int consumeLife = 0;
	{
		...
		if (curLife < needLife)
		{
			ch->ChatPacketTrans(CHAT_TYPE_INFO, "남은 생명력 양이 모자라 사용할 수 없습니다.");
			return -1;
		}
...
int CalculateConsumeSP(LPCHARACTER lpChar)
{
	...
	if (curSP < needSP)
	{
		lpChar->ChatPacketTrans(CHAT_TYPE_INFO, "남은 정신력 양이 모자라 사용할 수 없습니다.");
		return -1;
	}
...
bool CHARACTER::UseItemEx(LPITEM item, TItemPos DestCell)
{
	...
	{
		ChatPacketTrans(CHAT_TYPE_INFO, "대련 중에는 이용할 수 없는 물품입니다.");
		return false;
	}
#ifdef ENABLE_NEWSTUFF
		...
		case ITEM_CAMPFIRE:
			{
				...
				if (!tree)
				{
					ChatPacketTrans(CHAT_TYPE_INFO, "모닥불을 피울 수 없는 지점입니다.");
					return false;
				}

				if (tree->IsAttr((long)(GetX()+fx), (long)(GetY()+fy), ATTR_WATER))
				{
					ChatPacketTrans(CHAT_TYPE_INFO, "물 속에 모닥불을 피울 수 없습니다.");
					return false;
				}
								...
								switch (item->GetVnum())
								{
									case 71049: // 비단보따리
										if (g_bEnableBootaryCheck)
										{
											...
											{
												ChatPacketTrans(CHAT_TYPE_INFO, "개인 상점을 열 수 없는 지역입니다");
											}
										}
										else
										{
						...
						for (int i = 0; i < count; i++){
							switch (dwVnums[i])
							{
								case CSpecialItemGroup::GOLD:
									ChatPacketTrans(CHAT_TYPE_INFO, "돈 %d 냥을 획득했습니다.", dwCounts[i]);
									break;
								case CSpecialItemGroup::EXP:
									ChatPacketTrans(CHAT_TYPE_INFO, "상자에서 부터 신비한 빛이 나옵니다.");
									ChatPacketTrans(CHAT_TYPE_INFO, "%d의 경험치를 획득했습니다.", dwCounts[i]);
									break;
								case CSpecialItemGroup::MOB:
									ChatPacketTrans(CHAT_TYPE_INFO, "상자에서 몬스터가 나타났습니다!");
									break;
								case CSpecialItemGroup::SLOW:
									ChatPacketTrans(CHAT_TYPE_INFO, "상자에서 나온 빨간 연기를 들이마시자 움직이는 속도가 느려졌습니다!");
									break;
								case CSpecialItemGroup::DRAIN_HP:
									ChatPacketTrans(CHAT_TYPE_INFO, "상자가 갑자기 폭발하였습니다! 생명력이 감소했습니다.");
									break;
								case CSpecialItemGroup::POISON:
									ChatPacketTrans(CHAT_TYPE_INFO, "상자에서 나온 녹색 연기를 들이마시자 독이 온몸으로 퍼집니다!");
									break;
								...
								default:
									...
									{
										if (dwCounts[i] > 1)
											ChatPacketTrans(CHAT_TYPE_INFO, "상자에서 %s 가 %d 개 나왔습니다.", item_gets[i]->GetName(), dwCounts[i]);
										else
											ChatPacketTrans(CHAT_TYPE_INFO, "상자에서 %s 가 나왔습니다.", item_gets[i]->GetName());
		...
		case ITEM_GIFTBOX:
			{
				...
				if (0 != g_BoxUseTimeLimitValue)
				{
					...
					{
						ChatPacketTrans(CHAT_TYPE_INFO, "아직 골드를 버릴 수 없습니다.");
						return false;
					}
				}
		...
		case ITEM_SKILLFORGET:
			{
				...
				{
					ITEM_MANAGER::instance().RemoveItem(item);
					ChatPacketTrans(CHAT_TYPE_INFO, "스킬 레벨을 내리는데 성공하였습니다.");
				}
				else
					ChatPacketTrans(CHAT_TYPE_INFO, "스킬 레벨을 내릴 수 없습니다.");
			}
			break;
		...
		case ITEM_SKILLBOOK:
			{
				if (IsPolymorphed())
				{
					ChatPacketTrans(CHAT_TYPE_INFO, "변신중에는 책을 읽을수 없습니다.");
					return false;
				}
				...
				if (item->GetVnum() > 50800 && item->GetVnum() <= 50820)
				{
					...
					switch (item->GetSubType())
					{
						case USE_ABILITY_UP:
							...
							{
								ChatPacketTrans(CHAT_TYPE_INFO, "이미 효과가 걸려 있습니다.");
								return false;
							}
					...
					case USE_AFFECT :
						{
							...
							else
							{
								...
								if (item->IsPCBangItem() == true)
								{
									...
									{
										// PC방이 아님!
										ChatPacketTrans(CHAT_TYPE_INFO, "이 아이템은 PC방에서만 사용할 수 있습니다.");
										return false;
									}
								}
					...
					case USE_POTION_NODELAY:
						{
							if (CArenaManager::instance().IsArenaMap(GetMapIndex()) == true)
							{
								...
								{
									ChatPacketTrans(CHAT_TYPE_INFO, "대련장에서 사용하실 수 없습니다.");
									return false;
								}
								...
								switch (item->GetVnum())
								{
									...
									case 71020 :
										if (quest::CQuestManager::instance().GetEventFlag("arena_potion_limit_count") < 10000)
										{
											if (m_nPotionLimit <= 0)
											{
												ChatPacketTrans(CHAT_TYPE_INFO, "사용 제한량을 초과하였습니다.");
												return false;
											}
										}
										break;
							...
							if (used)
							{
								...
								{
									if (test_server)
										ChatPacketTrans(CHAT_TYPE_INFO, "월병 또는 종자 를 사용하였습니다");
						...
						switch (item->GetVnum())
						{
							...
							case ITEM_MARRIAGE_RING:
								{
									...
									if (pMarriage)
									{
										...
										ChatPacketTrans(CHAT_TYPE_INFO, "결혼 상태가 아니면 결혼반지를 사용할 수 없습니다.");
								}
								break;
							...
							case 50004: // 이벤트용 감지기
								{
									...
									else
									{
										...
										{
											ChatPacketTrans(CHAT_TYPE_INFO, "이 곳에선 이벤트용 감지기가 동작하지 않는것 같습니다.");
											return false;
										}
									}
									...
									{
										// 발견!
										ChatPacketTrans(CHAT_TYPE_INFO, "이벤트용 감지기가 신비로운 빛을 내며 사라집니다.");
									...
									{
										ChatPacketTrans(CHAT_TYPE_INFO, "이벤트용 감지기가 흔적도 없이 사라집니다.");
							...
							case 27987: // 조개
								...
								// 3   피진주 47994
								{
									...
									if (r <= 50)
									{
										ChatPacketTrans(CHAT_TYPE_INFO, "조개에서 돌조각이 나왔습니다.");
										AutoGiveItem(27990);
									}
									else
									{
										...
										if (r <= prob_table[0])
										{
											ChatPacketTrans(CHAT_TYPE_INFO, "조개가 흔적도 없이 사라집니다.");
										}
										else if (r <= prob_table[1])
										{
											ChatPacketTrans(CHAT_TYPE_INFO, "조개에서 백진주가 나왔습니다.");
											AutoGiveItem(27992);
										}
										else if (r <= prob_table[2])
										{
											ChatPacketTrans(CHAT_TYPE_INFO, "조개에서 청진주가 나왔습니다.");
											AutoGiveItem(27993);
										}
										else
										{
											ChatPacketTrans(CHAT_TYPE_INFO, "조개에서 피진주가 나왔습니다.");
											AutoGiveItem(27994);
										}
									}
								}
							...
							case 50303:
								{
									...
									{
										ChatPacketTrans(CHAT_TYPE_INFO, "이 책은 너무 어려워 이해하기가 힘듭니다.");
										return false;
									}

									if (lv >= item->GetValue(1))
									{
										ChatPacketTrans(CHAT_TYPE_INFO, "이 책은 아무리 봐도 도움이 될 것 같지 않습니다.");
										return false;
									}
							...
							case 50306:
								{
									...
									{
										ChatPacketTrans(CHAT_TYPE_INFO, "레벨 30이 되기 전에는 습득할 수 있을 것 같지 않습니다.");
										return false;
									}

									if (GetSkillLevel(SKILL_COMBO) == 1 && GetLevel() < 50)
									{
										ChatPacketTrans(CHAT_TYPE_INFO, "레벨 50이 되기 전에는 습득할 수 있을 것 같지 않습니다.");
										return false;
									}

									if (GetSkillLevel(SKILL_COMBO) >= 2)
									{
										ChatPacketTrans(CHAT_TYPE_INFO, "연계기는 더이상 수련할 수 없습니다.");
										return false;
									}
							...
							case 50313:
								{
									...
									{
										ChatPacketTrans(CHAT_TYPE_INFO, "이미 완벽하게 알아들을 수 있는 언어이다.");
										return false;
									}
							...
							case 50061 : // 일본 말 소환 스킬 수련서
								{
									...
									{
										ChatPacketTrans(CHAT_TYPE_INFO, "더 이상 수련할 수 없습니다.");
										return false;
									}
							...
							case 50325: case 50326: // 철통 수련서
								{
									...
									{
										ChatPacketTrans(CHAT_TYPE_INFO, "이 책을 읽으려면 레벨을 더 올려야 합니다.");
										return false;
									}
									...
									{
										ChatPacketTrans(CHAT_TYPE_INFO, "이 책으로는 더 이상 수련할 수 없습니다.");
										return false;
									}
							...
							case ITEM_HORSE_SKILL_TRAIN_BOOK:
								{
									...
									if (GetLevel() < 50)
									{
										ChatPacketTrans(CHAT_TYPE_INFO, "아직 승마 스킬을 수련할 수 있는 레벨이 아닙니다.");
										return false;
									}
									...
									{
										ChatPacketTrans(CHAT_TYPE_INFO, "더 이상 승마 수련서를 읽을 수 없습니다.");
										return false;
									}

									if (number(1, 100) <= iPct)
									{
										ChatPacketTrans(CHAT_TYPE_INFO, "승마 수련서를 읽어 승마 스킬 포인트를 얻었습니다.");
										ChatPacketTrans(CHAT_TYPE_INFO, "얻은 포인트로는 승마 스킬의 레벨을 올릴 수 있습니다.");
									...
									{
										ChatPacketTrans(CHAT_TYPE_INFO, "승마 수련서 이해에 실패하였습니다.");
									}
#ifdef ENABLE_BOOKS_STACKFIX
									item->SetCount(item->GetCount() - 1);
#else
							...
							case 70103: // 선두
								{
									...
									if (delta / 10 > 0)
									{
										...
										ChatPacketTrans(CHAT_TYPE_INFO, "선악치가 %d 증가하였습니다.", delta/10);
									}
								}
								break;
							...
							case 71107: // 천도복숭아
								{
									...
									{
										ChatPacketTrans(CHAT_TYPE_INFO, "선악치를 더 이상 올릴 수 없습니다.");
										return false;
									}
									...
									ChatPacket(CHAT_TYPE_TALKING, LC_TEXT("마음이 맑아지는군. 가슴을 짓누르던 무언가가 좀 가벼워진 느낌이야."));
									ChatPacketTrans(CHAT_TYPE_INFO, "선악치가 %d 증가하였습니다.", val);
							...
							case 72719:
								{
									...
									switch( item2->GetType() )
									{
										...
										case ITEM_ARMOR:
											...
											case ARMOR_WRIST:
											case ARMOR_NECK:
												ChatPacketTrans(CHAT_TYPE_INFO, "빼낼 영석이 없습니다");
												return false;
											}
											break;
							...
							case 70206:   // 염색약(검은색)
								{
									...
									{
										ChatPacketTrans(CHAT_TYPE_INFO, "현재 헤어스타일에서는 염색과 탈색이 불가능합니다.");
									}
									...
									else
									{
										...
										if (pPC)
										{
											...
											{
												ChatPacketTrans(CHAT_TYPE_INFO, "%d 레벨이 되어야 다시 염색하실 수 있습니다.", last_dye_level+3);
											}
										}
									}
								}
							...
							case ITEM_VALENTINE_CHOCOLATE:
								{
									...
									{
										// 성별이 맞지않아 쓸 수 없다.
										ChatPacketTrans(CHAT_TYPE_INFO, "성별이 맞지않아 이 아이템을 열 수 없습니다.");
										return false;
									}
							...
							case ITEM_ELK_VNUM: // 돈꾸러미
								{
									...
									ITEM_MANAGER::instance().RemoveItem(item);
									ChatPacketTrans(CHAT_TYPE_INFO, "돈 %d 냥을 획득했습니다.", iGold);
									ChangeGold(iGold);
								}
								break;
							...
							case 70021:
								{
									...
									if (CMonarch::instance().HealMyEmpire(this, HealPrice))
									{
										...
										ChatPacketTrans(CHAT_TYPE_INFO, "군주의 축복을 사용하였습니다.");
									}
								}
								break;
							...
							case 71051 : // 진재가
								{
									...
									{
										ChatPacketTrans(CHAT_TYPE_INFO, "속성을 변경할 수 없는 아이템입니다.");
										return false;
									}
									...
									{
										ChatPacketTrans(CHAT_TYPE_INFO, "성공적으로 속성이 추가 되었습니다");
									...
									{
										ChatPacketTrans(CHAT_TYPE_INFO, "더 이상 이 아이템으로 속성을 추가할 수 없습니다");
									}
								}
								break;
							...
							case 71052 : // 진재경
								{
									...
									{
										ChatPacketTrans(CHAT_TYPE_INFO, "변경 시킬 속성이 없습니다");
									}
								}
								break;
					...
					case USE_INVISIBILITY:
						{
							if (item->GetVnum() == 70026)
							{
								...
								if (pPC != NULL)
								{
									...
									{
										ChatPacketTrans(CHAT_TYPE_INFO, "아직 사용할 수 없습니다.");
										return false;
									}
					...
					case USE_TALISMAN:
						{
							...
							{
								ChatPacketTrans(CHAT_TYPE_INFO, "현재 위치에서 사용할 수 없습니다.");
								return false;
							}
							...
							if (m_pkWarpEvent)
							{
								ChatPacketTrans(CHAT_TYPE_INFO, "이동할 준비가 되어있음으로 귀환부를 사용할수 없습니다");
								return false;
							}
							...
							if (item->GetValue(0) == TOWN_PORTAL) // 귀환부
							{
								...
								{
									if (test_server)
										ChatPacketTrans(CHAT_TYPE_INFO, "원래 위치로 복귀");
							...
							else if (item->GetValue(0) == MEMORY_PORTAL) // 귀환기억부
							{
								if (item->GetSocket(0) == 0)
								{
									if (GetDungeon())
									{
										ChatPacketTrans(CHAT_TYPE_INFO, "던전 안에서는 %s%s 사용할 수 없습니다.",
												item->GetName(),
												"");
										return false;
									}
					...
					case USE_DETACHMENT:
						{
							...
							{
								ChatPacketTrans(CHAT_TYPE_INFO, "+3 영석은 이 아이템으로 개량할 수 없습니다");
								return false;
							}
							...
							if (item2->GetVnum() >= 28430 && item2->GetVnum() <= 28443)  // 영석+4
							{
								...
								{
									ChatPacketTrans(CHAT_TYPE_INFO, "영석은 이 아이템으로 개량할 수 없습니다");
								}
							}
							else
							{
					...
					case USE_RESET_COSTUME_ATTR:
						{
							...
							{
								ChatPacketTrans(CHAT_TYPE_INFO, "변경할 속성이 없습니다.");
								return false;
							}
							...
							ChatPacketTrans(CHAT_TYPE_INFO, "속성을 변경하였습니다.");
							...
							switch (item->GetSubType())
							{
								case USE_CLEAN_SOCKET:
									{
										...
										{
											ChatPacketTrans(CHAT_TYPE_INFO, "청소할 석이 박혀있지 않습니다.");
											return false;
										}
								...
								case USE_CHANGE_ATTRIBUTE2 : // @fixme123
									...
									if ((GM_PLAYER == GetGMLevel()) && (false == test_server) && (g_dwItemBonusChangeTime > 0))
									{
										...
										if (pPC)
										{
											...
											{
												ChatPacketTrans(CHAT_TYPE_INFO, "속성을 바꾼지 %d분 이내에는 다시 변경할 수 없습니다.(%d 분 남음)",
									...
									else
									{
										...
										if (item->GetVnum() == 71151 || item->GetVnum() == 76023)
										{
											...
											if (!bCanUse)
											{
												ChatPacketTrans(CHAT_TYPE_INFO, "적용 레벨보다 높아 사용이 불가능합니다.");
												break;
											}
										}
										item2->ChangeAttribute();
								...
								case USE_ADD_ATTRIBUTE :
									...
									if (item2->GetAttributeCount() < 4)
									{
										...
										{
											item2->AddAttribute();
											ChatPacketTrans(CHAT_TYPE_INFO, "속성 추가에 성공하였습니다.");
										...
										{
											ChatPacketTrans(CHAT_TYPE_INFO, "속성 추가에 실패하였습니다.");
									...
									{
										ChatPacketTrans(CHAT_TYPE_INFO, "더이상 이 아이템을 이용하여 속성을 추가할 수 없습니다.");
									}
									break;
								...
								case USE_ADD_ATTRIBUTE2 :
									...
									{
										ChatPacketTrans(CHAT_TYPE_INFO, "더 이상 이 아이템을 이용하여 속성을 추가할 수 없습니다.");
									}
									else if (item2->GetAttributeCount() < 4)
									{
										ChatPacketTrans(CHAT_TYPE_INFO, "먼저 재가비서를 이용하여 속성을 추가시켜 주세요.");
									}
									else
									{
										// wtf ?!
										...
										if (item2->IsAccessoryForSocket())
										{
											...
											if (item2->GetAccessorySocketMaxGrade() < ITEM_ACCESSORY_SOCKET_MAX_NUM)
											{
...
#endif
												{
													...
													ChatPacketTrans(CHAT_TYPE_INFO, "소켓이 성공적으로 추가되었습니다.");
													LogManager::instance().ItemLog(this, item, "ADD_SOCKET_SUCCESS", buf);
												}
												else
												{
													ChatPacketTrans(CHAT_TYPE_INFO, "소켓 추가에 실패하였습니다.");
											...
											{
												ChatPacketTrans(CHAT_TYPE_INFO, "이 액세서리에는 더이상 소켓을 추가할 공간이 없습니다.");
											}
										}
										else
										{
											ChatPacketTrans(CHAT_TYPE_INFO, "이 아이템으로 소켓을 추가할 수 없는 아이템입니다.");
										}
									}
									break;
									...
									if (item2->IsAccessoryForSocket() && item->CanPutInto(item2))
									{
										...
										if (item2->GetAccessorySocketGrade() < item2->GetAccessorySocketMaxGrade())
										{
											if (number(1, 100) <= aiAccessorySocketPutPct[item2->GetAccessorySocketGrade()])
											{
												...
												ChatPacketTrans(CHAT_TYPE_INFO, "장착에 성공하였습니다.");
												LogManager::instance().ItemLog(this, item, "PUT_SOCKET_SUCCESS", buf);
											}
											else
											{
												ChatPacketTrans(CHAT_TYPE_INFO, "장착에 실패하였습니다.");
										...
										else
										{
											if (item2->GetAccessorySocketMaxGrade() == 0)
												ChatPacketTrans(CHAT_TYPE_INFO, "먼저 다이아몬드로 악세서리에 소켓을 추가해야합니다.");
											else if (item2->GetAccessorySocketMaxGrade() < ITEM_ACCESSORY_SOCKET_MAX_NUM)
											{
												ChatPacketTrans(CHAT_TYPE_INFO, "이 액세서리에는 더이상 장착할 소켓이 없습니다.");
												ChatPacketTrans(CHAT_TYPE_INFO, "다이아몬드로 소켓을 추가해야합니다.");
											}
											else
												ChatPacketTrans(CHAT_TYPE_INFO, "이 액세서리에는 더이상 보석을 장착할 수 없습니다.");
										}
									}
									else
									{
										ChatPacketTrans(CHAT_TYPE_INFO, "이 아이템을 장착할 수 없습니다.");
									}
									break;
							}
							if (item2->IsEquipped())
					...
					case USE_BAIT:
						{
							...
							if (m_pkFishingEvent)
							{
								ChatPacketTrans(CHAT_TYPE_INFO, "낚시 중에 미끼를 갈아끼울 수 없습니다.");
								return false;
							}
							...
							{
								ChatPacketTrans(CHAT_TYPE_INFO, "이미 꽂혀있던 미끼를 빼고 %s를 끼웁니다.", item->GetName());
							}
							else
							{
								ChatPacketTrans(CHAT_TYPE_INFO, "낚시대에 %s를 미끼로 끼웁니다.", item->GetName());
							}
					...
					case USE_RECIPE :
						{
							...
							if (dwSourceCount1 != 0)
							{
								if (pSource1 == NULL)
								{
									ChatPacketTrans(CHAT_TYPE_INFO, "물약 조합을 위한 재료가 부족합니다.");
									return false;
								}
							}
							...
							if (pSource1 != NULL)
							{
								...
								{
									ChatPacketTrans(CHAT_TYPE_INFO, "재료(%s)가 부족합니다.", pSource1->GetName());
									return false;
								}
							...
							if (pSource2 != NULL)
							{
								...
								{
									ChatPacketTrans(CHAT_TYPE_INFO, "재료(%s)가 부족합니다.", pSource2->GetName());
									return false;
								}
							...
							{
								ChatPacketTrans(CHAT_TYPE_INFO, "빈 병이 모자릅니다.");
								return false;
							}
							...
							{
								ChatPacketTrans(CHAT_TYPE_INFO, "물약 제조에 실패했습니다.");
								return false;
							}
		...
		case ITEM_METIN:
			{
				...
				for (i = 0; i < ITEM_SOCKET_MAX_NUM; ++i)
				{
					...
					{
						ChatPacketTrans(CHAT_TYPE_INFO, "같은 종류의 메틴석은 여러개 부착할 수 없습니다.");
						return false;
					}
				}
				...
				if (item2->GetType() == ITEM_ARMOR)
				{
					...
					{
						ChatPacketTrans(CHAT_TYPE_INFO, "이 메틴석은 장비에 부착할 수 없습니다.");
						return false;
					}
				}
				else if (item2->GetType() == ITEM_WEAPON)
				{
					...
					{
						ChatPacketTrans(CHAT_TYPE_INFO, "이 메틴석은 무기에 부착할 수 없습니다.");
						return false;
					}
				}
				else
				{
					ChatPacketTrans(CHAT_TYPE_INFO, "부착할 수 있는 슬롯이 없습니다.");
					return false;
				}
...
#endif
						{
							ChatPacketTrans(CHAT_TYPE_INFO, "메틴석 부착에 성공하였습니다.");
							item2->SetSocket(i, item->GetVnum());
						}
						else
						{
							ChatPacketTrans(CHAT_TYPE_INFO, "메틴석 부착에 실패하였습니다.");
...
bool CHARACTER::UseItem(TItemPos Cell, TItemPos DestCell)
{
	...
	{
		ChatPacketTrans(CHAT_TYPE_INFO, "군직이 맞지않아 이 아이템을 사용할 수 없습니다.");
		return false;
	}
	...
	{
		ChatPacketTrans(CHAT_TYPE_INFO, "성별이 맞지않아 이 아이템을 사용할 수 없습니다.");
		return false;
	}
	...
	if (IS_SUMMON_ITEM(item->GetVnum()))
	{
		...
		{
			ChatPacketTrans(CHAT_TYPE_INFO, "사용할수 없습니다.");
			return false;
		}
		...
		{
			ChatPacketTrans(CHAT_TYPE_INFO, "삼거리 전투 참가중에는 귀환부,귀환기억부를 사용할수 없습니다.");
			return false;
		}
		...
		{
			ChatPacketTrans(CHAT_TYPE_INFO, "창고를 연후 %d초 이내에는 귀환부,귀환기억부를 사용할 수 없습니다.", g_nPortalLimitTime);
		...
		{
			ChatPacketTrans(CHAT_TYPE_INFO, "거래창,창고 등을 연 상태에서는 귀환부,귀환기억부 를 사용할수 없습니다.");
			return false;
		}
		...
		//개량후 시간체크
		{
			...
			{
				ChatPacketTrans(CHAT_TYPE_INFO, "아이템 개량후 %d초 이내에는 귀환부,귀환기억부를 사용할 수 없습니다.", g_nPortalLimitTime);
				return false;
			}
		}
		...
		//PREVENT_ITEM_COPY
		{
			...
			{
				ChatPacketTrans(CHAT_TYPE_INFO, "개인상점 사용후 %d초 이내에는 귀환부,귀환기억부를 사용할 수 없습니다.", g_nPortalLimitTime);
				return false;
			}
		...
		if (item->GetVnum() != 70302)
		{
			...
			if (nDistant > nDist)
			{
				ChatPacketTrans(CHAT_TYPE_INFO, "이동 되어질 위치와 너무 가까워 귀환부를 사용할수 없습니다.");
				if (test_server)
		...
		{
			ChatPacketTrans(CHAT_TYPE_INFO, "거래 후 %d초 이내에는 귀환부,귀환기억부등을 사용할 수 없습니다.", g_nPortalLimitTime);
			return false;
		}
	...
	if ((item->GetVnum() == 50200) || (item->GetVnum() == 71049))
	{
		if (GetExchange() || GetMyShop() || GetShopOwner() || IsOpenSafebox() || IsCubeOpen())
		{
			ChatPacketTrans(CHAT_TYPE_INFO, "거래창,창고 등을 연 상태에서는 보따리,비단보따리를 사용할수 없습니다.");
			return false;
		}
...
bool CHARACTER::DropItem(TItemPos Cell, BYTE bCount)
{
	...
	{
		ChatPacketTrans(CHAT_TYPE_INFO, "버릴 수 없는 아이템입니다.");
		return false;
	}
	...
	{
		ChatPacketTrans(CHAT_TYPE_INFO, "떨어진 아이템은 3분 후 사라집니다.");
#ifdef ENABLE_NEWSTUFF
...
#endif
			ChatPacketTrans(CHAT_TYPE_INFO, "떨어진 아이템은 %d분 후 사라집니다.", gold);
		}
...
bool CHARACTER::MoveItem(TItemPos Cell, TItemPos DestCell, BYTE count)
{
	...
	if (DestCell.IsEquipPosition())
	{
		...
		{
			ChatPacketTrans(CHAT_TYPE_INFO, "이미 장비를 착용하고 있습니다.");
	...
	if (item->DistanceValid(this))
	{
		...
		if (item->IsOwnership(this))
		{
			...
			else
			{
				if (item->IsStackable() && !IS_SET(item->GetAntiFlag(), ITEM_ANTIFLAG_STACK))
				{
					...
					for (int i = 0; i < INVENTORY_MAX_NUM; ++i)
					{
						...
						if (item2->GetVnum() == item->GetVnum())
						{
							...
							if (bCount == 0)
							{
								ChatPacketTrans(CHAT_TYPE_INFO, "아이템 획득: %s", item2->GetName());
								M2_DESTROY_ITEM(item);
				...
				if ((iEmptyCell = GetEmptyInventory(item->GetSize())) == -1)
				{
					...
					ChatPacketTrans(CHAT_TYPE_INFO, "소지하고 있는 아이템이 너무 많습니다.");
					return false;
				}
				...
				ChatPacketTrans(CHAT_TYPE_INFO, "아이템 획득: %s", item->GetName());
		...
		else if (!IS_SET(item->GetAntiFlag(), ITEM_ANTIFLAG_GIVE | ITEM_ANTIFLAG_DROP) && GetParty())
		{
			...
			if (!(owner && (iEmptyCell = owner->GetEmptyInventory(item->GetSize())) != -1))
			{
				...
				if ((iEmptyCell = GetEmptyInventory(item->GetSize())) == -1)
				{
					owner->ChatPacketTrans(CHAT_TYPE_INFO, "소지하고 있는 아이템이 너무 많습니다.");
					return false;
				}
			}
			...
			{
				owner->ChatPacketTrans(CHAT_TYPE_INFO, "아이템 획득: %s 님으로부터 %s", GetName(), item->GetName());
				ChatPacketTrans(CHAT_TYPE_INFO, "아이템 전달: %s 님에게 %s", owner->GetName(), item->GetName());
			}
...
bool CHARACTER::EquipItem(LPITEM item, int iCandidateCell)
{
	...
	{
		ChatPacketTrans(CHAT_TYPE_INFO, "말을 탄 상태에서 예복을 입을 수 없습니다.");
		return false;
	}

	if (iWearCell != WEAR_ARROW && IsPolymorphed())
	{
		ChatPacketTrans(CHAT_TYPE_INFO, "둔갑 중에는 착용중인 장비를 변경할 수 없습니다.");
		return false;
	}
	...
	{
		ChatPacketTrans(CHAT_TYPE_INFO, "이미 탈것을 이용중입니다.");
		return false;
	}
	...
	{
		ChatPacketTrans(CHAT_TYPE_INFO, "가만히 있을 때만 착용할 수 있습니다.");
		return false;
	}
	...
	switch (GetRaceNum())
	{
		...
		case 20103:
			...
			if (item->GetVnum() == ITEM_REVIVE_HORSE_1)
			{
				if (!IsDead())
				{
					from->ChatPacketTrans(CHAT_TYPE_INFO, "죽지 않은 말에게 선초를 먹일 수 없습니다.");
					return false;
				}
				return true;
			}
			else if (item->GetVnum() == ITEM_HORSE_FOOD_1)
			{
				if (IsDead())
				{
					from->ChatPacketTrans(CHAT_TYPE_INFO, "죽은 말에게 사료를 먹일 수 없습니다.");
					return false;
				}
				return true;
			}
...
void CHARACTER::ReceiveItem(LPCHARACTER from, LPITEM item)
{
	...
	switch (GetRaceNum())
	{
		...
		case DEVILTOWER_BLACKSMITH_ACCESSORY_MOB:
			...
			{
				from->ChatPacketTrans(CHAT_TYPE_INFO, "이 아이템은 개량할 수 없습니다.");
			}
			break;
		...
		case 20109:
					...
					item->GetVnum() == ITEM_REVIVE_HORSE_3)
			{
				from->ReviveHorse();
				item->SetCount(item->GetCount()-1);
				from->ChatPacketTrans(CHAT_TYPE_INFO, "말에게 선초를 주었습니다.");
			}
			else if (item->GetVnum() == ITEM_HORSE_FOOD_1 ||
			...
			{
				from->FeedHorse();
				from->ChatPacketTrans(CHAT_TYPE_INFO, "말에게 사료를 주었습니다.");
				item->SetCount(item->GetCount()-1);
				EffectPacket(SE_HPUP_RED);
			}
			break;
...
bool CHARACTER::ItemProcess_Hair(LPITEM item, int iDestCell)
{
	...
	{
		// 레벨 제한에 걸림
		ChatPacketTrans(CHAT_TYPE_INFO, "아직 이 머리를 사용할 수 없는 레벨입니다.");
		return false;
	}
	...
	{
		ChatPacketTrans(CHAT_TYPE_INFO, "동일한 머리 스타일로는 교체할 수 없습니다.");
		return true;
	}
...
bool CHARACTER::ItemProcess_Polymorph(LPITEM item)
{
	if (IsPolymorphed())
	{
		ChatPacketTrans(CHAT_TYPE_INFO, "이미 둔갑중인 상태입니다.");
		return false;
	}

	if (true == IsRiding())
	{
		ChatPacketTrans(CHAT_TYPE_INFO, "둔갑할 수 없는 상태입니다.");
		return false;
	}
	...
	if (dwVnum == 0)
	{
		ChatPacketTrans(CHAT_TYPE_INFO, "잘못된 둔갑 아이템입니다.");
		item->SetCount(item->GetCount()-1);
		return false;
	}
		...
		case 71093 :
			{
				...
				{
					ChatPacketTrans(CHAT_TYPE_INFO, "나보다 너무 높은 레벨의 몬스터로는 변신 할 수 없습니다.");
					return false;
				}
...
bool CHARACTER::CanEquipNow(const LPITEM item, const TItemPos& srcCell, const TItemPos& destCell) /*const*/
{
	...
	for (int i = 0; i < ITEM_LIMIT_MAX_NUM; ++i)
	{
		...
		switch (itemTable->aLimits[i].bType)
		{
			case LIMIT_LEVEL:
				if (GetLevel() < limit)
				{
					ChatPacketTrans(CHAT_TYPE_INFO, "레벨이 낮아 착용할 수 없습니다.");
					return false;
				}
				break;
			...
			case LIMIT_STR:
				...
				{
					ChatPacketTrans(CHAT_TYPE_INFO, "근력이 낮아 착용할 수 없습니다.");
					return false;
				}
				break;
			...
			case LIMIT_INT:
				...
				{
					ChatPacketTrans(CHAT_TYPE_INFO, "지능이 낮아 착용할 수 없습니다.");
					return false;
				}
				break;
			...
			case LIMIT_DEX:
				...
				{
					ChatPacketTrans(CHAT_TYPE_INFO, "민첩이 낮아 착용할 수 없습니다.");
					return false;
				}
				break;
			...
			case LIMIT_CON:
				...
				{
					ChatPacketTrans(CHAT_TYPE_INFO, "체력이 낮아 착용할 수 없습니다.");
					return false;
				}
				break;
		}
	...
	if (item->GetWearFlag() & WEARABLE_UNIQUE)
	{
		...
		{
			ChatPacketTrans(CHAT_TYPE_INFO, "같은 종류의 유니크 아이템 두 개를 동시에 장착할 수 없습니다.");
			return false;
		}
		...
		{
			ChatPacketTrans(CHAT_TYPE_INFO, "결혼하지 않은 상태에서 예물을 착용할 수 없습니다.");
			return false;
		}
