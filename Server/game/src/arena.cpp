...
EVENTFUNC(ready_to_start_event)
{
	...
	if (chA == NULL || chB == NULL)
	{
		...
		if (chA != NULL)
		{
			chA->ChatPacketTrans(CHAT_TYPE_INFO, "대련 상대가 사라져 대련을 종료합니다.");
			sys_log(0, "ARENA: Oppernent is disappered. MyPID(%d) OppPID(%d)", pArena->GetPlayerAPID(), pArena->GetPlayerBPID());
		}

		if (chB != NULL)
		{
			chB->ChatPacketTrans(CHAT_TYPE_INFO, "대련 상대가 사라져 대련을 종료합니다.");
	...
	switch (info->state)
	{
		case 0:
			{
				...
				if (count > 10000)
				{
					chA->ChatPacketTrans(CHAT_TYPE_INFO, "물약 제한이 없습니다.");
					chB->ChatPacketTrans(CHAT_TYPE_INFO, "물약 제한이 없습니다.");
				}
				else
				{
					...
					chA->ChatPacketTrans(CHAT_TYPE_INFO, "물약을 %d 개 까지 사용 가능합니다.", chA->GetPotionLimit());
					chB->ChatPacketTrans(CHAT_TYPE_INFO, "물약을 %d 개 까지 사용 가능합니다.", chB->GetPotionLimit());
				}
				chA->ChatPacketTrans(CHAT_TYPE_INFO, "10초뒤 대련이 시작됩니다.");
				chB->ChatPacketTrans(CHAT_TYPE_INFO, "10초뒤 대련이 시작됩니다.");
		...
		case 1:
			{
				chA->ChatPacketTrans(CHAT_TYPE_INFO, "대련이 시작되었습니다.");
				chB->ChatPacketTrans(CHAT_TYPE_INFO, "대련이 시작되었습니다.");
		...
		default:
			{
				chA->ChatPacketTrans(CHAT_TYPE_INFO, "대련장 문제로 인하여 대련을 종료합니다.");
				chB->ChatPacketTrans(CHAT_TYPE_INFO, "대련장 문제로 인하여 대련을 종료합니다.");
...
bool CArena::OnDead(DWORD dwPIDA, DWORD dwPIDB)
{
	...
	else if (pCharA != NULL && pCharB != NULL)
	{
		if (m_dwPIDA == dwPIDA)
		{
			...
			{
				restart = true;
				pCharA->ChatPacketTrans(CHAT_TYPE_INFO, "%s 님이 승리하였습니다.", pCharA->GetName());
				pCharA->ChatPacket(CHAT_TYPE_NOTICE, "%s %d : %d %s", pCharA->GetName(), m_dwSetPointOfA, m_dwSetPointOfB, pCharB->GetName());

				pCharB->ChatPacketTrans(CHAT_TYPE_INFO, "%s 님이 승리하였습니다.", pCharA->GetName());
		...
		else if (m_dwPIDB == dwPIDA)
		{
			...
			{
				restart = true;
				pCharA->ChatPacketTrans(CHAT_TYPE_INFO, "%s 님이 승리하였습니다.", pCharB->GetName());
				pCharA->ChatPacket(CHAT_TYPE_NOTICE, "%s %d : %d %s", pCharA->GetName(), m_dwSetPointOfA, m_dwSetPointOfB, pCharB->GetName());

				pCharB->ChatPacketTrans(CHAT_TYPE_INFO, "%s 님이 승리하였습니다.", pCharB->GetName());
				pCharB->ChatPacket(CHAT_TYPE_NOTICE, "%s %d : %d %s", pCharA->GetName(), m_dwSetPointOfA, m_dwSetPointOfB, pCharB->GetName());
	...
	{
		if (pCharA != NULL)
			pCharA->ChatPacketTrans(CHAT_TYPE_INFO, "10초뒤 마을로 되돌아갑니다.");

		if (	pCharB != NULL)
			pCharB->ChatPacketTrans(CHAT_TYPE_INFO, "10초뒤 마을로 되돌아갑니다.");
	...
	{
		if (pCharA != NULL)
			pCharA->ChatPacketTrans(CHAT_TYPE_INFO, "10초뒤 다음 판을 시작합니다.");

		if (pCharB != NULL)
			pCharB->ChatPacketTrans(CHAT_TYPE_INFO, "10초뒤 다음 판을 시작합니다.");
...
void CArena::OnDisconnect(DWORD pid)
{
	...
	{
		if (GetPlayerB() != NULL)
			GetPlayerB()->ChatPacketTrans(CHAT_TYPE_INFO, "상대방 캐릭터가 접속을 종료하여 대련을 중지합니다.");
	...
	{
		if (GetPlayerA() != NULL)
			GetPlayerA()->ChatPacketTrans(CHAT_TYPE_INFO, "상대방 캐릭터가 접속을 종료하여 대련을 중지합니다.");
