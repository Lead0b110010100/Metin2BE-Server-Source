...
EVENTFUNC(ready_to_start_event)
{
	...
	if (chA == NULL || chB == NULL)
	{
		...
		pArena->SendChatPacketToObserver(CHAT_TYPE_NOTICE, LC_TEXT("Duel finished as your combatant vanished."));
...
EVENTFUNC(duel_time_out)
{
	...
	if (chA == NULL || chB == NULL)
	{
		...
		pArena->SendChatPacketToObserver(CHAT_TYPE_INFO, LC_TEXT("Duel finished as your combatant vanished."));
	...
	else
	{
		...
		{
			case 0:
				pArena->SendChatPacketToObserver(CHAT_TYPE_NOTICE, LC_TEXT("The duel is finished because of the timeout."));
				pArena->SendChatPacketToObserver(CHAT_TYPE_NOTICE, LC_TEXT("In 10 seconds you will be transferred into the city."));

				chA->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("The duel is finished because of the timeout."));
				chA->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("In 10 seconds you will be transferred into the city."));

				chB->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("The duel is finished because of the timeout."));
				chB->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("In 10 seconds you will be transferred into the city."));
...
bool CArena::OnDead(DWORD dwPIDA, DWORD dwPIDB)
{
	...
	{
		// 둘다 접속이 끊어졌다 ?!
		SendChatPacketToObserver(CHAT_TYPE_NOTICE, LC_TEXT("The duel is finished because there are problems with the arena."));
		restart = false;
	}
	else if (pCharA == NULL && pCharB != NULL)
	{
		pCharB->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("Your Combatant has problems. The duel is finished."));
		SendChatPacketToObserver(CHAT_TYPE_NOTICE, LC_TEXT("The duel is finished as there is a problem with the Combatant."));
		restart = false;
	}
	else if (pCharA != NULL && pCharB == NULL)
	{
		pCharA->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT("Your Combatant has problems. The duel is finished."));
		SendChatPacketToObserver(CHAT_TYPE_NOTICE, LC_TEXT("The duel is finished as there is a problem with the Combatant."));
		restart = false;
	}
	else if (pCharA != NULL && pCharB != NULL)
