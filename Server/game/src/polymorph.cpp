...
bool CPolymorphUtils::PolymorphCharacter(LPCHARACTER pChar, LPITEM pItem, const CMob* pMob)
{
	...
	if (iPolyPercent <= 0)
	{
		pChar->ChatPacketTrans(CHAT_TYPE_INFO, "You have failed to transform.");
		return false;
	}
	else
	{
...
bool CPolymorphUtils::UpdateBookPracticeGrade(LPCHARACTER pChar, LPITEM pItem)
{
	...
	if (pItem->GetSocket(1) > 0)
		...
		pChar->ChatPacketTrans(CHAT_TYPE_INFO, "You have finished your training. Please get it from your master.");
