...
void CHARACTER::DeathPenalty(BYTE bTown)
{
	...
	if (GetLevel() < 10)
	{
		...
		ChatPacketTrans(CHAT_TYPE_INFO, "용신의 가호로 경험치가 떨어지지 않았습니다.");
		return;
	}
		...
		if (!IS_SET(m_pointsInstant.instant_flag, INSTANT_FLAG_NO_REWARD))
		{
			...
			else
			{
				if (pkKiller->m_dwUnderGuildWarInfoMessageTime < get_dword_time())
				{
					...
					pkKiller->ChatPacketTrans(CHAT_TYPE_INFO, "<길드> 길드전중에는 사냥에 따른 이익이 없습니다.");
				}
			}
		}
	}
...
bool CHARACTER::Damage(LPCHARACTER pAttacker, int dam, EDamageType type) // returns true if dead
{
		...
		if (pAttacker)
		{
			...
			if (iPenetratePct)
			{
				...
				if (number(1, 100) <= iPenetratePct)
				{
					...
					if (test_server)
						ChatPacketTrans(CHAT_TYPE_INFO, "관통 추가 데미지 %d", GetPoint(POINT_DEF_GRADE) * (100 + GetPoint(POINT_DEF_BONUS)) / 100);
	...
	else if (type == DAMAGE_TYPE_NORMAL || type == DAMAGE_TYPE_NORMAL_RANGE)
	{
		if (type == DAMAGE_TYPE_NORMAL)
		{
			...
			if (GetPoint(POINT_BLOCK) && number(1, 100) <= GetPoint(POINT_BLOCK))
			{
				if (test_server)
				{
					pAttacker->ChatPacketTrans(CHAT_TYPE_INFO, "%s 블럭! (%d%%)", GetName(), GetPoint(POINT_BLOCK));
					ChatPacketTrans(CHAT_TYPE_INFO, "%s 블럭! (%d%%)", GetName(), GetPoint(POINT_BLOCK));
				}
		...
		else if (type == DAMAGE_TYPE_NORMAL_RANGE)
		{
			...
			if (GetPoint(POINT_DODGE) && number(1, 100) <= GetPoint(POINT_DODGE))
			{
				if (test_server)
				{
					pAttacker->ChatPacketTrans(CHAT_TYPE_INFO, "%s 회피! (%d%%)", GetName(), GetPoint(POINT_DODGE));
					ChatPacketTrans(CHAT_TYPE_INFO, "%s 회피! (%d%%)", GetName(), GetPoint(POINT_DODGE));
				}
	...
	if (pAttacker)
	{
		...
		if (m_bDetailLog)
		{
			ChatPacketTrans(CHAT_TYPE_INFO, "%s[%d]가 공격 위치: %d %d", pAttacker->GetName(), (DWORD) pAttacker->GetVID(), pAttacker->GetX(), pAttacker->GetY());
		}
	}
