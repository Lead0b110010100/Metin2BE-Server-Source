...
ACMD(do_monarch_notice)
{
	...
	{
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "군주만이 사용 가능한 기능입니다");
	}
}
...
ACMD(do_makeguild)
{
	...
	gm.CreateGuild(cp);
	ch->ChatPacketTrans(CHAT_TYPE_INFO, "(%s) 길드가 생성되었습니다. [임시]", cp.name);
}
...
ACMD(do_guild_state)
{
	...
	{
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "%s: 존재하지 않는 길드 입니다.", arg1);
	}
}
...
ACMD(do_priv_guild)
{
	...
	if (*arg1)
	{
		...
		if (!g)
			ch->ChatPacketTrans(CHAT_TYPE_INFO, "그런 이름 또는 번호의 길드가 없습니다.");
		else
		{
			char buf[1024+1];
...
ACMD(do_block_chat_list)
{
	...
	{
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "그런 명령어는 없습니다");
		return;
	}
		...
		case 'c':
			{
				...
				if (!t)
				{
					ch->ChatPacketTrans(CHAT_TYPE_INFO, "존재하지 않는 건물입니다.");
					return;
				}
				...
				if (t->dwGroupVnum)
				{
					...
					{
						ch->ChatPacketTrans(CHAT_TYPE_INFO, "같이 지을 수 없는 종류의 건물이 지어져 있습니다.");
						return;
					}
				}
				...
				if (t->dwDependOnGroupVnum)
				{
					...
					//		if (dependent)
					{
						...
						{
							ch->ChatPacketTrans(CHAT_TYPE_INFO, "건설에 필요한 건물이 지어져 있지 않습니다.");
							return;
						}
					}
				}
				...
				if (test_server || GMLevel == GM_PLAYER)
				{
					...
					{
						ch->ChatPacketTrans(CHAT_TYPE_INFO, "건물 비용 정보 이상으로 건설 작업에 실패했습니다.");
						return;
					}

					if (ch->GetGold() < (int)t->dwPrice)
					{
						ch->ChatPacketTrans(CHAT_TYPE_INFO, "건설 비용이 부족합니다.");
						return;
					}
					...
					for (i = 0; i < OBJECT_MATERIAL_MAX_NUM; ++i)
					{
						...
						{
							ch->ChatPacketTrans(CHAT_TYPE_INFO, "자재가 부족하여 건설할 수 없습니다.");
							return;
						}
					}
				}
				...
				if (!isSuccess)
				{
					ch->ChatPacketTrans(CHAT_TYPE_INFO, "건물을 지을 수 없는 위치입니다.");
					return;
				}
				else
				{
...
ACMD(do_horse_level)
{
	...
	if (NULL == victim)
	{
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "존재하지 않는 캐릭터 입니다.");
		return;
	}
...
ACMD(do_end_duel)
{
	...
	{
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "대련 강제 종료 실패");
	}
	else
	{
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "대련 강제 종료 성공");
	}
}
...
ACMD(do_duel)
{
	...
	if (pChar1 != NULL && pChar2 != NULL)
	{
		...
		if (pParty != NULL)
		{
			...
			{
				pChar1->ChatPacketTrans(CHAT_TYPE_INFO, "<파티> 파티에서 나가셨습니다.");
		...
		if (pParty != NULL)
		{
			...
			{
				pChar2->ChatPacketTrans(CHAT_TYPE_INFO, "<파티> 파티에서 나가셨습니다.");
		...
		{
			ch->ChatPacketTrans(CHAT_TYPE_INFO, "대련이 성공적으로 시작 되었습니다.");
		}
		else
		{
			ch->ChatPacketTrans(CHAT_TYPE_INFO, "대련 시작에 문제가 있습니다.");
		}
	}
	else
	{
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "대련자가 없습니다.");
	}
}
...
ACMD(do_stat_plus_amount)
{
	...
	if (ch->IsPolymorphed())
	{
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "둔갑 중에는 능력을 올릴 수 없습니다.");
		return;
	}
	...
	if (nRemainPoint <= 0)
	{
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "남은 스탯 포인트가 없습니다.");
		return;
	}
	...
	{
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "남은 스탯 포인트가 적습니다.");
		return;
	}

	if (nPoint < 0)
	{
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "값을 잘못 입력하였습니다.");
		return;
	}
	...
	switch (subcmd)
	{
		...
		default :
			ch->ChatPacketTrans(CHAT_TYPE_INFO, "명령어의 서브 커맨드가 잘못 되었습니다.");
			return;
			break;
	}
#endif
...
ACMD(do_break_marriage)
{
	...
	ch->ChatPacketTrans(CHAT_TYPE_INFO, "플레이어 %d 와 플레이어  %d를 파혼시킵니다..", pids.pid1, pids.pid2);
...
{
	ch->ChatPacketTrans(CHAT_TYPE_INFO, "각제국 진행 정보");
	ch->ChatPacketTrans(CHAT_TYPE_INFO, "선택 맵 정보 성지 %d 통로 %d %d %d", GetSungziMapIndex(), GetPassMapIndex(1), GetPassMapIndex(2), GetPassMapIndex(3));
...
{
	ch->ChatPacketTrans(CHAT_TYPE_INFO, "나의 삼거리 진행정보");
...
ACMD(do_set_stat)
{
	...
	{
		ch->ChatPacketTrans(CHAT_TYPE_INFO, "Invalid argument.");
		return;
	}
	...
	if (!tch)
	{
		...
		if (pkCCI)
		{
			ch->ChatPacketTrans(CHAT_TYPE_INFO, "Cannot find player(%s). %s is not in your game server.", szName, szName);
			return;
		}
		else
		{
			ch->ChatPacketTrans(CHAT_TYPE_INFO, "Cannot find player(%s). Perhaps %s doesn't login or exist.", szName, szName);
			return;
		}
	}
	else
